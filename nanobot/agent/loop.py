...
class AgentLoop:
    ...
    def __init__(
        ...
        self.plugins = self.context.plugins  # reuse ContextBuilder's PluginLoader
    ...
    async def _process_message(
        ...
        if cmd == "/help":
            help_lines = ["🐈 nanobot commands:", "/new — Start a new conversation", "/stop — Stop the current task", "/help — Show available commands"]
            plugin_cmd_lines = []
            ...
            skill_lines = []
            for s in self.context.skills.list_skills(filter_unavailable=True):
                desc = self.context.skills._get_skill_description(s["name"])
                skill_lines.append(f"/{s['name']} — {desc}")
            if skill_lines:
                help_lines.append("\nSkill commands:")
                help_lines.extend(skill_lines)
            return OutboundMessage(channel=msg.channel, chat_id=msg.chat_id,
                                   content="\n".join(help_lines))
        ...
        if stripped.startswith("/"):
            ...
            if cmd_name not in ("new", "help", "stop"):
                ...
                if plugin_cmd is not None:
                    ...
                    # Skill slash command fallback
                    skill_content = self.context.skills.load_skill(cmd_name)
                    if skill_content is not None:
                        stripped = self.context.skills._strip_frontmatter(skill_content)
                        user_note = f" {cmd_args}" if cmd_args.strip() else ""
                        msg = dataclasses.replace(
                            msg,
                            content=f"[Skill: /{cmd_name}{user_note}]\n\n{stripped}",
                        )
                        logger.info("Expanded skill /{} -> {} chars", cmd_name, len(stripped))
        ...
        if msg.metadata:
            initial_messages[-1]["metadata"] = msg.metadata
